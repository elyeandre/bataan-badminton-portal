...
exports.updateAdminEvent = async (req, res) => {
  try {
    ...
    if (!event.court.equals(user.court) && !event.postedBy.equals(user.id)) {
      return res.status(403).json({ status: 'error', message: 'You can only edit your own events.' });
    }

    const { heading, details, startDate, endDate, reservationFee, eventFee, participantLimit } = req.body;
    
    // Update fields if provided
    if (heading) event.heading = heading;
    if (details) event.details = details;
    if (startDate) event.startDate = moment.tz(startDate, 'Asia/Manila').toDate();
    if (endDate) event.endDate = moment.tz(endDate, 'Asia/Manila').toDate();
    if (reservationFee !== undefined) event.reservationFee = reservationFee;
    if (eventFee !== undefined) event.eventFee = eventFee;
    ...
    if (req.files && req.files.images) {
      ...
      if (Array.isArray(images)) {
        imagesUrls = await handleMultipleFileUploads(images, user.id, 'eventImage', allowedImages);
      } else {
        imagesUrls.push(await handleFileUpload(images, user.id, 'eventImage', allowedImages));
      }
...
exports.updateAdminAnnouncement = async (req, res) => {
  try {
    const user = req.user;
    ...
    if (!announcement.court.equals(user.court) && !announcement.postedBy.equals(user.id)) {
      return res.status(403).json({ status: 'error', message: 'You can only edit your own announcements.' });
    }
    ...
    if (details) announcement.details = details;
    ...
    if (req.files && req.files.images) {
      ...
      if (Array.isArray(images)) {
        imagesUrls = await handleMultipleFileUploads(images, user.id, 'announcementImage', allowedImages);
      } else {
        imagesUrls.push(await handleFileUpload(images, user.id, 'announcementImage', allowedImages));
      }

      // Replace existing images with new ones
      if (imagesUrls.length > 0) {
        announcement.images = imagesUrls;
      }
    }

    await announcement.save();
    return res.status(200).json({ status: 'success', data: announcement });
  } catch (err) {
    error(err);
    return res.status(500).json({ status: 'error', message: 'Internal Server Error' });
  }
...
exports.createPost = async (req, res, io) => {
  try {
    ...
    if (content && content.trim().length > 0) {
      ...
      normalizedHashtags = hashtags.map((tag) => tag.toLowerCase().substring(1));
    }
    ...
    if (imageFile) {
      try {
        ...
        imageUrl = await handleFileUpload(
          ...
          'post_images',
          allowedImageTypes,
          [], // no document types for posts
          [] // no other types for posts
        );
      } catch (uploadError) {
        return res.status(400).json({
          status: 'error',
    ...
    // update the Hashtag collection only if there are hashtags
...
exports.retrieveAllPosts = async (req, res) => {
  try {
    ...
    const posts = await Post.find(query)
      .populate('userId', '_id username email role profile_photo')
      .sort(sortCriteria)
...
exports.removePost = async (req, res, io) => {
  try {
    ...
    // emit websocket event so other clients refresh
    if (io) {
      io.emit('postDeleted', { status: 'success', postId });
    }
...
exports.addLike = async (req, res, io) => {
  try {
    ...
    // find the post by ID and populate user info
...
exports.addComment = async (req, res) => {
  try {
    ...
    const commenter = await User.findById(userId, 'username');
    
    if (parentComment) {
      // This is a reply - notify the parent comment author
      const parentCommentObj = post.comments.id(parentComment);
      if (parentCommentObj && parentCommentObj.userId.toString() !== userId.toString()) {
        await createNotification(
          parentCommentObj.userId,
          'New Reply',
      ...
      // This is a direct comment - notify the post owner
      if (post.userId._id.toString() !== userId.toString()) {
        await createNotification(
          post.userId._id,
          'New Comment',
...
exports.fetchComments = async (req, res) => {
  try {
    ...
    // find the post by ID
    const post = await Post.findById(postId).populate('comments.userId', '_id username profile_photo');
...
exports.likeComment = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      data: { likesCount: comment.likesCount, likedBy: comment.likedBy }
    });
  } catch (err) {
...
exports.editPost = async (req, res) => {
  try {
    ...
    const { content } = req.body;
    const imageFile = req.files ? req.files.image : null;
    const { removeImage } = req.body; // Flag to remove the existing image

    // Check if either content or image is provided
    if ((!content || content.trim().length === 0) && !imageFile && !post.image && removeImage !== 'true') {
      return res.status(400).json({
        status: 'error',
        message: 'Either content or an image is required for the post'
      });
    }

    // find the post by ID
    const post = await Post.findById(postId).populate('userId', 'username profile_photo');
    ...
    // Delete old image file if we're replacing it or removing it
    if (oldImagePath) {
      try {
    ...
    // Add socket event for post edit
    if (req.io) {
      req.io.emit('postEdited', {
        status: 'success',
        postId: post._id,
...
exports.updateCourtOwnerInfo = async (req, res) => {
  try {
    const userId = req.user.id;
    
    // Ensure the user is an admin (court owner)
    if (!req.user.isAdmin) {
      return res.status(403).json({
        status: 'error',
        message: 'Only court owners can update court information.'
      });
    }

    // Find the court associated with this user
    const court = await Court.findOne({ user: userId });
    if (!court) {
      return res.status(404).json({
        status: 'error',
        message: 'Court not found for this user.'
      });
    }
    ...
    if (operating_hours) {
      try {
        updateData.operating_hours = JSON.parse(operating_hours);
      } catch (e) {
        updateData.operating_hours = operating_hours;
      }
    }
    if (hourly_rate) updateData.hourly_rate = hourly_rate;
    if (paypal_email) updateData.paypal_email = paypal_email;
    if (dti_number) updateData.dti_number = dti_number;
    if (description) updateData.description = description;

    // Handle file uploads for business logo
    if (req.files && req.files.business_logo) {
      const allowedImages = ['image/jpeg', 'image/png', 'image/gif'];
      const businessLogoUrl = await handleFileUpload(req.files.business_logo, userId, 'businessLogo', allowedImages);
      updateData.business_logo = businessLogoUrl;
    }

    // Handle multiple court images
    if (req.files && req.files['court_image[]']) {
      const allowedImages = ['image/jpeg', 'image/png', 'image/gif'];
      const courtImageUrls = await handleMultipleFileUploads(
        req.files['court_image[]'],
        userId,
        'courtImage',
        allowedImages
      );
      updateData.court_images = courtImageUrls;
    }

    // Handle facility images and names
    if (req.files && req.body.facility_name) {
      const facilityNames = Array.isArray(req.body.facility_name) ? req.body.facility_name : [req.body.facility_name];
      const facilityImages = req.files['facility_image[]'] ? 
        (Array.isArray(req.files['facility_image[]']) ? req.files['facility_image[]'] : [req.files['facility_image[]']]) : [];
      
      if (facilityImages.length > 0 && facilityImages.length === facilityNames.length) {
        const allowedImages = ['image/jpeg', 'image/png', 'image/gif'];
        const facilityData = [];
        
        for (let i = 0; i < facilityImages.length; i++) {
          const imageUrl = await handleFileUpload(facilityImages[i], userId, 'facilityImage', allowedImages);
          facilityData.push({
            name: facilityNames[i],
            image: imageUrl
          });
        }
        updateData.facilities = facilityData;
      }
    }

    // Handle document uploads
    const documentKeys = ['business_permit', 'dti', 'bir', 'sanitary_permit', 'barangay_clearance', 'non_coverage', 'dole_registration'];
    const allowedDocs = [
      'application/pdf',
      'application/msword',
    ...
    for (const key of documentKeys) {
      const documentKey = `documents[${key}]`;
      const file = req.files[documentKey];

      if (file) {
        if (Array.isArray(file)) {
          // Handle multiple files
          updateData[`documents.${key}`] = await Promise.all(
            file.map((f) => handleFileUpload(f, userId, 'businessDocuments', allowedDocs))
          );
        } else {
          // Handle single file
          updateData[`documents.${key}`] = await handleFileUpload(file, userId, 'businessDocuments', allowedDocs);
        }
      }
    }

    // Update the court document
    ...
    console.error('Error updating court owner info:', err);
