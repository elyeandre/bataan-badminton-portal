...
const { startMongoKeepAlive } = require('./src/utils/mongoKeepAlive');
const { startHashtagCleanupCronJob } = require('./src/utils/hashtagCleanup');
...
const server = httpServer.listen(config.get('port'), config.get('host'), () => {
  ...
  startMongoKeepAlive(); // start the MongoDB keep-alive cron job
  // start the reservation cleanup cron job
  startReservationCleanupCronJob();
  // start the hashtag cleanup cron job
  startHashtagCleanupCronJob();
